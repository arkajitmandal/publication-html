...
external library dependencies:
    ...
    feedparser
    numpy 
    tqdm (optional, but tracks progress of script)
...
def main():
    ...
    with open(output_file, 'w') as outfile:
        ...
        for year_i, jsons_year_i in groupby(published_json, key=sort_year_function):
            outfile.write(f'<h2>{year_i}</h2>')
            outfile.write("\n")
            #outfile.write('<ol class="pubs">')
            outfile.write("\n")
            for json_j in jsons_year_i:
                outfile.write(f'<p value="{article_num}">')
                outfile.write(f'{article_num}. ')
                outfile.write("\n")
